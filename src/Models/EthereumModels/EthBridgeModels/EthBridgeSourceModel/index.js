...
import {
  EthBridgeSourceNetworkSelect,
  EthBridgeMessage,
} from "../../../../redux/actions/EthereumActions/EthBridgeActions";
import EthBridgeModelsWrapper from "../EthBridgeModels.style";
import { EthBridgeSourceNetworkList } from "../../../../assets/api/Ethereum/EthBridge";

const EthBridgeSourceModel = ({
  ethBridgeSourceModel,
  setEthBridgeSourceModel,
}) => {
  const dispatch = useDispatch();

  const EthBridgeSourceModelRemoveOverLay = (val) => {
    dispatch(EthBridgeSourceNetworkSelect(val));
    ...
    setTimeout(() => {
      setEthBridgeSourceModel(false);
    }, 500);
  };

  useEffect(() => {
    if (ethBridgeSourceModel) {
      var Bridge_overlay = document.getElementById("Bridge_overlay");
      var Bridge_popup = document.getElementById("Bridge_popup");
                  ...
                  <div className="col-lg-4 col-2 d-flex justify-content-end">
                    <div className="close_div">
                      <RiCloseCircleLine
                        ...
                        onClick={() => setEthBridgeSourceModel(false)}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="col-12 mt-2">
                    <div className="network_list">
                      <div className="row" id="token_list">
                        {EthBridgeSourceNetworkList.map((val) => {
                          return (
                            <div className="col-12" key={val.id} id="tokens">
                              <div
                                ...
                                onClick={
                                  val.id === 1 || val.id === 2
                                    ? () =>
                                        EthBridgeSourceModelRemoveOverLay(val)
...
export default EthBridgeSourceModel;
