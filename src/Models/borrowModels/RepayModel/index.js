...
const RepayModel = ({ repayModel, setRepayModel }) => {
  const dispatch = useDispatch();

  const RepayTokenApiNew = RepayTokenApi();

  const RepayModelFilterFunction = () => {
  ...
  const removeOverLay = (img, name) => {
    if (img && name) {
      dispatch(RepayTokenSelect(img, name));
    }
                  ...
                  <div className="col-12">
                    <div className="search_box">
                      <input
                        ...
                        onKeyUp={() => RepayModelFilterFunction()}
                      />
                    </div>
                  </div>
