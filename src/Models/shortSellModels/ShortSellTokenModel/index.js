...
import { shortSellTokenAssetsApi } from "../../../assets/api/ShortSellApis";
import { ShortSellTokenSelect } from "../../../redux/actions/ShortSellActions";
import ModelSWrapper from "../../Model.style";

const ShortSellTokenModel = ({
  shortSellTokenModel,
  setShortSellTokenModel,
}) => {
  const dispatch = useDispatch();

  const filterShortSellTokenModel = () => {
  ...
  const removeOverLay = (img, name) => {
    if (img && name) {
      dispatch(ShortSellTokenSelect(img, name));
    }
    ...
    setTimeout(() => {
      setShortSellTokenModel(false);
    }, 500);
  };

  useEffect(() => {
    if (shortSellTokenModel) {
      var overlay = document.getElementById("overlay");
      var popup = document.getElementById("popup");
                  ...
                  <div className="col-lg-4 col-2 d-flex justify-content-end">
                    <div className="close_div">
                      <RiCloseCircleLine
                        ...
                        onClick={() => setShortSellTokenModel(false)}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="col-12">
                    <div className="search_box">
                      <input
                        ...
                        onKeyUp={() => filterShortSellTokenModel()}
                      />
                    </div>
                  </div>
                  ...
                  <div className="col-12 mt-3">
                    <div className="token_list">
                      <div className="row" id="token_list">
                        {shortSellTokenAssetsApi.map((val) => {
                          return (
...
export default ShortSellTokenModel;
