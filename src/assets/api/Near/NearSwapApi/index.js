...
export const NearSwapTokenApi = (NearTokenPriceArr) => {
  var NearSwapTokenApiNew = [];

  for (var i = 0; i < NearSwapTokens?.length; i++) {
    for (var j = 0; j < NearTokenPriceArr?.length; j++) {
      if (NearSwapTokens[i].name === NearTokenPriceArr[j].name) {
        NearSwapTokenApiNew.push({
          ...NearSwapTokens[i],
          Bal: 0,
